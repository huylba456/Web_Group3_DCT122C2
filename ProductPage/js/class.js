    class user {
        ...
        static login(username, password) {
            ...
            if (isUserid) {
                 history.addNode(Date.now(), "Người dùng " + username + " đã đăng nhập!", username, user.getUserId(isUserid).email);
    ...
    class history {
        ...
        static addNode(name,email,timeSpan, textShow) {
            var list = [];
            if (history.getList()) {
                list = history.getList();
            }
            list.push(new History(name, email, timeSpan,textShow));
    ...
    class order {
        ...
        static makeOrder(cartList, userid, orderTime) {
            ...
            var userz = user.getUserId(userid);
            history.addNode(userz.name, userz.email, Date.now(), "Người dùng " + user.getUserId(userid).username + " đã đặt 1 đơn hàng!");
            return true;
        }
